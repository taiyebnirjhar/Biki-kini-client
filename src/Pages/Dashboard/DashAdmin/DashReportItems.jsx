...
import DashReportedItemsRow from "../../../components/DashFields/DashReportedItemsRow";
import { AuthContext } from "../../../Contexts/FireAuthContext";

function DashReportItems() {
  ...
  const url = `${import.meta.env.VITE_API_URL}/report`;

  const { data: reportedIt = [], refetch } = useQuery({
    queryKey: ["reportedIt", user.uid],
    queryFn: async () => {
  ...
  const handleDeleteReportedItems = (id) => {
    const prodId = id;
    ...
    fetch(`${import.meta.env.VITE_API_URL}/products/myproducts/${id}`, {
      ...
      .then((res) => res.json())
      .then(() => {
        handleDeleteItemsFromBuyer(prodId);
      })
      .then(() => {
  ...
  const handleDeleteREPORTE = (id) => {
    fetch(`${import.meta.env.VITE_API_URL}/report/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleDeleteItemsFromBuyer = (id) => {
    fetch(`${import.meta.env.VITE_API_URL}/admin/delete/myproducts/${id}`, {
      method: "DELETE",
    })
      .then((res) => res.json())
      .then(() => {})
        ...
        <aside className="w-full md:w-64" aria-label="Sidebar">
          <div className="overflow-y-auto py-4 px-3 bg-gray-50 rounded dark:bg-gray-800">
            <ul className="space-y-3">
              ...
              <li>
                <Link to={"/dashboard/admin/reportedItems"}></Link>
                <div className="flex items-center p-2 text-base font-normal text-gray-900 rounded-lg dark:text-white hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer">
                  <div className="flex-shrink-0  text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white">
                    <span className="h-full w-full">
          ...
          <div className="overflow-x-auto relative shadow-md sm:rounded-lg">
            <table className="w-full text-sm text-left text-gray-500 dark:text-gray-400">
              ...
              return  />;
                ...
                {/* {user_DATA.map((items) => (
              <DashAllUsersRow key={items._id} userData={items} />
            ))} */}

                {reportedIt.map((items) => (
                  <DashReportedItemsRow
                    key={items._id}
                    userData={items}
                    handleDeleteReportedItems={handleDeleteReportedItems}
                    handleDeleteREPORTE={handleDeleteREPORTE}
                  />
                ))}
              </tbody>
            </table>
...
export default DashReportItems;
